...
export async function POST(request: NextRequest) {
  try {
    ...
    const { success, data, error } = await googleServices.getUserInfo()

    if (success) {
      return NextResponse.json({ success: true, data })
    } else {
      return NextResponse.json({ success: false, error: error || 'Failed to get user info' }, { status: 500 })
    }
  } catch (err) {
    console.error('Error in /api/google/test-connection:', err)
